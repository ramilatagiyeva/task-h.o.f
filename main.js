...
// 1) height in 150 den boyuk olanlari filterden isdifade ederek tapin
  const tallPeople = users.filter(person => person.height > 150);
...
// arrayden melumatlari tapib ekrana yazsin meselen getName("Addison") Addison a aid objecti logda gorsetsin
  function getName(Emma) {
    const person = users.find(p => p.Emma === Emma);
    if (person) {
      console.log(person);
    } else {
     console.log(${Emma} adli shexs tapilmadi.);
    }
  }
  getName("Addison");
